...
Expect<int32_t> SwsScale::body(const Runtime::CallingFrame &,uint32_t SwsCtxId,uint32_t InputFrameId,int32_t SrcSliceY,int32_t SrcSliceH,uint32_t OutputFrameId){
  ...
  return sws_scale(SwsCtx,InputFrame->data,InputFrame->linesize,SrcSliceY,SrcSliceH,OutputFrame->data,OutputFrame->linesize);
}
...
Expect<int32_t> SwsGetDefaultFilter::body(const Runtime::CallingFrame &Frame,uint32_t SwsFilterPtr,float LumaGBlur,float ChromaGBlur,float LumaSharpen,float ChromaSharpen,float ChromaHShift,float ChromaVShift,int32_t Verbose){

  MEMINST_CHECK(MemInst,Frame,0);
  MEM_PTR_CHECK(SwsFilterId,MemInst,uint32_t,SwsFilterPtr,"")

  SwsFilter* Filter = sws_getDefaultFilter(LumaGBlur,ChromaGBlur,LumaSharpen,ChromaSharpen,ChromaHShift,ChromaVShift,Verbose);
  if(Filter == NULL)
    return static_cast<int32_t>(ErrNo::InternalError);
...
Expect<int32_t> SwsAllocVec::body(const Runtime::CallingFrame &Frame,uint32_t SwsVectorPtr,int32_t Length){
...
Expect<int32_t> SwsGetGaussianVec::body(const Runtime::CallingFrame &Frame,uint32_t SwsVectorPtr,double Variance,double Quality){
...
Expect<int32_t> SwsScaleVec::body(const Runtime::CallingFrame &,uint32_t SwsVectorId,double Scalar){
...
Expect<int32_t> SwsNormalizeVec::body(const Runtime::CallingFrame &,uint32_t SwsVectorId,double Height){
...
Expect<int32_t> SwsFreeVec::body(const Runtime::CallingFrame &,uint32_t SwsVectorId){
